...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x_Q, x_K, x_V, src_batch_lens=None):
        ...
        attention = attention / torch.sqrt(d_k) 
        ...
        attention = self.dropout(attention) # Using implementation guide in original paper
...
class TF_Encoder_Block(nn.Module):
    ...
    def forward(self, x, src_batch_lens):
        ...
        out1 = self.layer_norm1(x + multi)
        feed_forward = self.feed_forward(out1)
        out = self.layer_norm2(out1 + feed_forward)
        return out
...
class TF_Encoder(nn.Module):
    def __init__(self, vocab_size, d_model,
        ...
        #     *([TF_Encoder_Block(d_model, d_ff, numhead, dropout)] * numlayer)
        # )
